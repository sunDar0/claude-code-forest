...
- **10단계 성장**: 그날 사용량 비율(일일누적 역대최대 대비)로 묘목 1~3 · 유목 1~3 · 성목 1~4 프레임이 결정됩니다.
...
- **시간대 하늘**: 로컬 시각에 따라 하늘색이 하루를 돕니다(새벽 보라 → 정오 파랑 → 황혼 주황 → 야간 남색).
- **이번 달 진행**: 우상단 HUD 에 "이번 달 숲 N/M일" 막대. 달이 자동으로 묶이는 순간 완성 토스트가 잠깐 뜹니다.
- **상세**: 칸을 클릭하면 모달 상단에 그 칸을 확대한 도트 미리보기, 하단에 날짜·단계·수종·경험치·5메트릭(각 줄에 평균 대비 침엽수 뱃지 — 초록=평균 위·붉은 역삼각형=평균 아래)이 표시됩니다.

### 사용량 → 나무 매핑 (요약)

| 사용량 | 나무 표현 |
|--------|-----------|
| 일일 총 토큰 / 일일누적 역대최대 | 성장 단계(10단계) |
...
- **나무 단계 분모**: 일일누적 토큰의 **역대 최대**(`dailyMaxTokens`, 단조 증가). 역대급으로 쓴 날이 성목 만개가 됩니다. 아래 일일 cap 과는 별개 값입니다.
- **일일 cap(HUD·5h 표시용)**: statusline 5시간 사용률 역산값과 5h 블록 totals 의 **p85 분위수** 중 큰 값 × 2.
  관측 최대(max)가 아니라 분위수라, 한도 리셋 같은 이상치 한 날이 이 값을 영구히 부풀리지 않습니다.
