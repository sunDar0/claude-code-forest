...
description: Claude Code Forest 의 전반적인 UI 형태·완성도·배치가 기획 의도에 부합하는지 라이브 화면으로 점검하고, 어긋난 곳을 안전하게 교정하는 워크플로우를 조율한다. ui-design-reviewer(라이브 캡처·기획 대조 점검) → 승인 게이트 → forest-game-builder(교정) → 재점검. "UI 점검", "UI 검토", "완성도 점검", "배치 점검", "기획 부합 점검", "디자인 검토", "화면 점검" 작업 시 반드시 이 스킬을 사용. "다시 점검", "재점검", "그 항목만 교정", "회귀 확인", "이전 리뷰 기반" 같은 후속 요청에도 사용. 단순 질문은 직접 응답 가능.
---
...
**서브 에이전트 파이프라인 + 승인 게이트.** 점검 → (사용자 승인) → 교정 → 재점검의 순차 의존. 팀 통신 불필요.
...
스킬: `procedural-dot-forest`(렌더) 등. 사용자가 고른 항목만 교정. 렌더 변경이면 골든 갱신(의도적), `npm test` 통과. 데이터·서버 불변 원칙 유지. **교정 후 라이브 재확인은 reviewer 가**(builder 가 self-검증 후에도 독립 재점검).

## Phase 5: 재점검 (ui-design-reviewer)

교정 항목을 **다시 라이브 캡처**해 해소됐는지 + 회귀 없는지 확인. `_workspace/60_ui_review.md` 갱신.

## 데이터 전달 프로토콜

- **파일 기반**(주): `_workspace/60_ui_review.md` + 스크린샷 근거. 코드 교정은 실제 파일.
- **반환값 기반**: 서브 에이전트 완료 메시지.
- 교정 항목은 메인이 builder 프롬프트에 실어 전달(어느 발견·기획 기준·기대 결과).
