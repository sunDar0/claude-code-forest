...
const STATUSLINE_CACHE = path.join(os.homedir(), '.claude', 'statusline', 'usage-cache.json');
...
async function readPct(key, altKey) {
  let raw;
  try {
    raw = await readFile(STATUSLINE_CACHE, 'utf8');
  } catch {
 ...
 */
export async function scanUsageData() {
  ...
  const cutoffTime = new Date(Date.now() - WINDOW_DAYS * 24 * 60 * 60 * 1000);

  // Pass 1: 중복 제거 (key -> { dateStr, usage })
  const messageData = {};
  ...
  for (const file of files) {
    ...
    for (const line of lines) {
      ...
      if (msgTime < cutoffTime) continue; // 90일 윈도우.

      const message = entry.message || {};
      const usage = message.usage;
      if (!usage || typeof usage !== 'object' || Object.keys(usage).length === 0) continue;

      const dateStr = localYMD(msgTime);
      const key = message.id || `no_id_${timestamp}`;

      // 항상 덮어씀 — 마지막 엔트리가 최종 토큰 값을 가짐.
      // ms(블록 묶기용 timestamp)도 같이 보관 — 5시간 블록 추정에서 메시지 단위로 재사용한다.
      messageData[key] = { dateStr, usage, ms: msgTime.getTime() };
    }
  }
  ...
  for (const { dateStr, usage, ms } of Object.values(messageData)) {
    const d =
      dailyStats[dateStr] ||
      (dailyStats[dateStr] = {
        input: 0,
    ...
    blockMessages.push({ ms, totalTokens: input + output + cacheWrite + cacheRead });
  }
